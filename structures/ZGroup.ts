...
export default class BaseZGroup extends BaseGroup {
    async getLessonsStartDate(): Promise<Date | undefined> {
        let date = (await GroupModel.findOne({ name: this.name }).exec())?.lessonsStartDate;
        ...
        if(!date && this.cachedFullRawSchedule?.data?.length) {
            ...
            }, undefined as unknown as ILesson);
    ...
    async getDayRawSchedule(date: Date | number = new Date()) {
        ...
        else return fullSchedule.filter((p) => 'datez' in p.day && p.day.datez == date.toISOString().slice(0, 10)).sort((a, b) => a.number - b.number);
    }
}
